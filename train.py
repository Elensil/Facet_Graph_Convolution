...
def run_thingies(f_normals_list, GTfn_list, f_adj_list, images_lists, calibs_lists, valid_f_normals_list, valid_GTfn_list, valid_f_adj_list,valid_images_lists,valid_calibs_lists):
	...
	with tf.variable_scope("model"):
		...
		n_conv = get_appearance_model_reg(fn_, fadj0, ARCHITECTURE, keep_prob,images_,calibs_)
	# n_conv = normalizeTensor(n_conv)
		...
		for iter in range(NUM_ITERATIONS):
			...
			# Compute validation loss
				...
				for vbm in range(valid_samp):
					...
					valid_fd = {fn_: valid_f_normals_list[vbm], fadj0: valid_f_adj_list[vbm][0], fadj1: valid_f_adj_list[vbm][1], fadj2: valid_f_adj_list[vbm][2], tfn_: valid_GTfn_list[vbm],
...
def mainFunction():
	...
	binDumpPath = "/morpheo-nas/marmando/DeepMeshRefinement/TrainingBase/BinaryDump/WithImages/"
	...
	#Takes the path to noisy and GT meshes as input, and add data to the lists fed to tensroflow graph, with the right format
	def addMeshWithImages(inputFilePath,filename, gtFilePath, gtfilename, calibFilePath, imagesFilePath, in_list, gt_list, adj_list, im_list, cal_list, mesh_count_list):
		...
		loaded_calibs = []
		for cam_calib in os.listdir(calibFilePath):
			cam_calib_file=calibFilePath+cam_calib
		...
		loaded_images = []
		for cam_image_folder in os.listdir(imagesFilePath):
			image_file_name = imagesFilePath+cam_image_folder+"/0001.png"
		...
		if facesNum>maxSize:
			...
			for p in range(patchNum):
				...
				gt_list.append(GTf_normals)
				#Add calib + images
				im_list.append(loaded_images)
				cal_list.append(loaded_calibs)
	...
	if running_mode == 0:
		...
		if pickleLoad:
			...
			# Training set
			for meshDir in os.listdir(inputFilePath):
				#print("training_meshes_num start_iter " + str(training_meshes_num))
				if training_meshes_num[0]>300:
					break
				print("Adding " + meshDir + " (" + str(training_meshes_num[0]) + ")")

				smoothFileName=meshDir+"/Smooth/000.obj"
				gtfilename = meshDir+"/Ground_Truth/000.obj"
				calibFilePath = inputFilePath+'/'+meshDir+"/Calib/"
				imagesFilePath = inputFilePath+'/'+meshDir+"/Images/"
				addMeshWithImages(inputFilePath, smoothFileName, inputFilePath, gtfilename, calibFilePath, imagesFilePath,
					f_normals_list, GTfn_list, f_adj_list, images_lists, calibs_lists, training_meshes_num)


			# Validation set
			for meshDir in os.listdir(validFilePath):

				smoothFileName=meshDir+"/Smooth/000.obj"
				gtfilename = meshDir+"/Ground_Truth/000.obj"
				calibFilePath = validFilePath+'/'+meshDir+"/Calib/"
				imagesFilePath = validFilePath+'/'+meshDir+"/Images/"
				addMeshWithImages(inputFilePath, smoothFileName, inputFilePath, gtfilename, calibFilePath, imagesFilePath,
					valid_f_normals_list, valid_GTfn_list, valid_f_adj_list, valid_images_lists, valid_calibs_lists, valid_meshes_num)
		...
		# Loop over meshes
		for meshDir in os.listdir(DataFolder):
			nameArray = []
			resultsArray = []
			gtFolder=DataFolder+meshDir+"/Ground_Truth/"
			noisyFolder=DataFolder+meshDir+"/Smooth/"
			gtFileName ="000.obj"
			# Get smooth meshes
			noisyFile0 = "000.obj"
			denoizedFile0 = meshDir+"_denoized.obj"
			...
			print("running ...")
			upV, upN0 = run_thingy(V0, faces_noisy_modif, f_normals_pos0, f_adj_noisy, edge_map_noisy, v_e_map_noisy,[loaded_images],[loaded_calibs])
