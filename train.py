...
def inferNet(in_points, f_normals, f_adj, edge_map, v_e_map,images_lists,calibs_lists,num_wofake_nodes,patch_indices,old_to_new_permutations,num_faces):
            ...
            if facesNum>maxSize:
                while(np.any(faceCheck==0)):
                    ...
                    if np.sum(np.abs(GTf_normals)) > 0.0:
                        ...
                        print("Added patch: mesh " + filename + ", patch (" + str(mesh_count_list[0]) + ")")
                        mesh_count_list[0]+=1
                    else:
                        print("Found only fake nodes graph, discarding!")
                        exit()
